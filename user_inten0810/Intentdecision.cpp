	  ...
	  pssit = -1;
		...
		if(pssit >= 0)
		{
			   //本次参数
				...
				if(pb == NULL)//上次没有
				{  
					   //上一次没场景// 现在参数——> cmbtable 
					   if(gnum == 0)
					   {
					   	   *psenck = 0;
								 strcpy(psenck, "NULL");
					   	   *presfdback = 0;
			         	 sprintf(presfdback, "主人您上次就没有场景, 主人，小胖不知道你在说什么，玩点别的！");
			         	 *Ituval = 0;
			         	 if(ptgut != NULL)
										free(ptgut);
								 if(pcmbar != NULL)
																  ...
																  *ptgut = 0;
																  ...
																  if(zhbit >= 0)
											  					{
											  						   *psenck = 0;
																	 		 strcpy(psenck, "NULL");
											         	   	   *presfdback = 0;
																		   strcat(presfdback, "上次的场景为空，可到达的场景：");
																		   strcat(presfdback, pd->desckey);
									 	 	   		           strcat(presfdback, ",");
						...
						while(pb != NULL)
						{
							  if(gnum == 0)
							  {
							  	  *psenck = 0;
								 		strcpy(psenck, "NULL");
							  	  *presfdback = 0;
				         	 	sprintf(presfdback, "主人您上次%s场景, 主人，小胖不知道你在说什么，玩点别的吧", pb->psenckey);
				         	 	*Ituval = 0;
				         	 	if(ptgut != NULL)
												free(ptgut);
									 	if(pcmbar != NULL)
							  	  ...
							  	  if(pb->paranum <= 0)
									  {
					   	   	      ...
					   	   	      *psenck = 0;
										 		 strcpy(psenck, "NULL");
				         	   	   *presfdback = 0;
											   sprintf(presfdback, "主人您上次%s场景：本次可到达", pb->psenckey);
					         	   	 pd = NULL;
											 	 pd = (cptable+(*parmsit))->fdlist;
											 	 while(pd!= NULL)
											 	 {
																    	   ...
																    	   *ptgut = 0;
																			   ...
																			   if(zhbit >= 0)
														  					 {
														  					 	    *psenck = 0;
																							strcpy(psenck, "NULL");
																						  *presfdback = 0;
																			        sprintf(presfdback, "主人您上次%s场景, 这次可以切换到%s场景", pb->psenckey, (cmbtable+zhbit)->psenckey);
	  ...
	  *bfsenck = 0;
	  ...
	  else
	  {
	  	  ...
	  	  bfitval = 0;
				...
				jsbv = -1;
			  *pfdback = 0;
				jsbv = Dfbackusintent(bfsenck, bfitval, schash, windtable, mwhash, root, mroot, PWarry, palist, psenck, Ituval, pfdback);//提取参数后,全部命中 子串匹配 字符串逻辑与相似.
...
///////////////////////////////////////////////////////////
//function: 当次用户述求为“是的“-->切换场景 Determine switch scenarios
//parameter:
//author: wuxiaoqi
//time: 2016-6-23
///////////////////////////////////////////////////////////
char * Deterswitchscen(hashsptable hashmp, hashinfotabl cptable, hashploctable clohash, char * probotid, char * kuquery, char * psenck, int *Ituval, char * presfdback)
{
    ...
    if( strcmp(kuquery, "好的")== 0 ||strcmp(kuquery, "是的")== 0||strcmp(kuquery, "ok")== 0||strcmp(kuquery, "OK")== 0||strcmp(kuquery, "可以切换")== 0)
    {
    	  pcmbar = (char *)malloc(2048);
	      *pcmbar = 0;
